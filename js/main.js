...
function getNumberFromFormattedString(str) {
    ...
    // Проверяем, является ли число отрицательным (содержит минус)
    const isNegative = str.toString().trim().startsWith('-');
    // Удаляем все нецифровые символы
    const numStr = str.toString().replace(/\D/g, '');
    // Преобразуем в число с учетом знака
    return parseInt(numStr) * (isNegative ? -1 : 1) || 0;
}
...
function generateRecommendations() {
    ...
    recommendations.push({
        ...
        text: `Максимально используйте инструменты с налоговыми преимуществами, такие как ИИС. При максимальном годовом взносе 400 тыс. ₽ вы получите налоговый вычет 52 тыс. ₽ в год, что существенно повысит доходность ваших инвестиций.`
    });
